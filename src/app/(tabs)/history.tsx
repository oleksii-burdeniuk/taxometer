...
export default function HistoryScreen() {
  ...
  return <SafeAreaView edges={['top']} style={sharedStyles.screen}>
    <ScreenHeader
      title={t('history')}
      action={<Pressable accessibilityRole="button" onPress={() => router.push('/statistics')} style={styles.statsButton}><Ionicons name="bar-chart-outline" size={18} color={colors.dark} /><Text style={styles.statsButtonText}>{t('statistics')}</Text></Pressable>}
    />
...
const createStyles = (colors: ThemeColors) => StyleSheet.create({
  statsButton: { minHeight: 38, flexDirection: 'row', alignItems: 'center', gap: 6, paddingHorizontal: 12, borderRadius: 14, backgroundColor: colors.primary },
  statsButtonText: { color: colors.dark, fontSize: 12, fontWeight: '900' },
  emptyContainer: { flexGrow: 1, justifyContent: 'center' },
  empty: { alignItems: 'center', gap: 9, marginBottom: 80 },
  emptyIcon: { width: 76, height: 76, borderRadius: 24, backgroundColor: colors.surfaceAlt, alignItems: 'center', justifyContent: 'center', marginBottom: 6 },
  emptyTitle: { color: colors.text, fontSize: 22, fontWeight: '800' },
  trip: { flexDirection: 'row', alignItems: 'center', gap: 8, padding: 10 },
  tripMain: { flex: 1, minWidth: 0, flexDirection: 'row', alignItems: 'center', gap: 12, padding: 4 },
  dateIcon: { width: 48, height: 50, borderRadius: 14, backgroundColor: colors.surfaceAlt, alignItems: 'center', justifyContent: 'center' },
  day: { color: colors.text, fontSize: 17, fontWeight: '900' },
  month: { color: colors.muted, fontSize: 9, fontWeight: '800' },
  tripInfo: { flex: 1, gap: 4 },
  tripName: { color: colors.text, fontSize: 16, fontWeight: '800' },
  tripMeta: { color: colors.muted, fontSize: 12 },
  price: { maxWidth: 105, color: colors.text, fontSize: 15, fontWeight: '900', textAlign: 'right' },
  deleteButton: { width: 38, height: 38, borderRadius: 12, backgroundColor: colors.dangerSoft, alignItems: 'center', justifyContent: 'center' },
});
