...
import { Button, Card, ScreenHeader, Stat, useSharedStyles } from '@/components/ui';
...
export default function MeterScreen() {
  ...
  const { ready, tariffs, selectedTariffId, setSelectedTariffId, activeTrip, displayTrip, elapsedSeconds, gpsStale, recommendedPeriod, zoneMode, setZoneMode, startTrip, togglePause, finishTrip, switchTripTariff } = useApp();
      ...
      <ScrollView contentContainerStyle={sharedStyles.content} showsVerticalScrollIndicator={false}>
        {activeTrip ? (
          <>
            ...
            {(activeTrip.trackingWarning === 'gps' || gpsStale) && <View style={styles.gpsWarning}><Ionicons name="warning-outline" size={18} color={colors.warningText} /><Text style={styles.gpsWarningText}>{t('gpsBillingPaused')}</Text></View>}
...
const createStyles = (colors: ThemeColors) => StyleSheet.create({
  ...
  tariff: { minHeight: 82, paddingHorizontal: 15, paddingVertical: 14, borderRadius: 19, backgroundColor: colors.surface, borderWidth: 1.5, borderColor: colors.border, flexDirection: 'row', alignItems: 'center', gap: 12 },
  tariffSelected: { borderColor: colors.primary, backgroundColor: colors.selected }, tariffPressed: { opacity: 0.78 }, tariffInfo: { flex: 1, gap: 6 },
  ...
  selector: { width: 22, height: 22, borderRadius: 11, borderWidth: 1.5, borderColor: colors.muted, alignItems: 'center', justifyContent: 'center' }, selectorSelected: { backgroundColor: colors.dark, borderColor: colors.dark },
  ...
  segmented: { flexDirection: 'row', padding: 4, gap: 4, borderRadius: 15, backgroundColor: colors.surfaceAlt },
  segment: { flex: 1, minHeight: 42, paddingHorizontal: 8, borderRadius: 12, alignItems: 'center', justifyContent: 'center' },
  segmentActive: { backgroundColor: colors.dark }, segmentText: { color: colors.muted, fontSize: 13, fontWeight: '800', textAlign: 'center' }, segmentTextActive: { color: colors.onDark },
  segmentDisabled: { opacity: 0.45 }, gpsWarning: { flexDirection: 'row', alignItems: 'center', gap: 8, padding: 13, borderRadius: 14, backgroundColor: colors.warningBackground }, gpsWarningText: { flex: 1, color: colors.warningText, fontSize: 12, fontWeight: '800' },
  ...
  selectionGroup: { gap: 8, padding: 9, borderRadius: 20, backgroundColor: colors.surfaceAlt }, groupLabel: { paddingHorizontal: 7, paddingTop: 3, color: colors.muted, fontSize: 11, fontWeight: '900', textTransform: 'uppercase' }, routeModeCard: { gap: 9, padding: 14, borderRadius: 18, backgroundColor: colors.surface, borderWidth: 1, borderColor: colors.border },
