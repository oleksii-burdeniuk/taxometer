...
export type ReceiptLabels = {
  ...
  includedAllowance: string; thankYou: string; notFiscal: string;
};
...
export async function createReceipt(trip: Trip, locale: string, labels: ReceiptLabels) {
  ...
  <section class="meta">
    ...
    <div class="meta-row"><span>${escapeHtml(labels.tariff)}</span><b>${escapeHtml((tariffNames.length ? tariffNames : [trip.tariff.name]).join(' → '))}</b></div>
  </section>
  <hr class="cut">
  <div class="summary"><span><b>${(trip.distanceMeters / 1000).toFixed(2)} km</b>${escapeHtml(labels.distance)}</span><span><b>${formatDuration(duration)}</b>${escapeHtml(labels.time)}</span><span><b>${formatDuration(trip.waitingSeconds)}</b>${escapeHtml(labels.waiting)}</span></div>
  <hr class="cut">
  <div class="line"><span>${escapeHtml(labels.baseCharge)}</span><b>${money(fare.baseCharge)}</b></div>
  ...
  ${allowanceRow}${minimumRow}<hr class="cut">
